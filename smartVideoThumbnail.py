...
def measure_similarity_blur():
    global grey, previousBestImage, previousBestImageCount, previousBestImageBlur
    ...
    if (score > 0.6):
        ...
        if (previousBestImageBlur < fm):
            print(
                "choosing current frame %d with previous blur %d current blur %d" % (count, previousBestImageBlur, fm))
...
def detect_eyes_smile():
    ...
    for (x, y, w, h) in faces:
        ...
        for faceClassifier in faceClassifiers:
            ...
            face_features = facial_feature_cascade.detectMultiScale(roi_grey, 1.1, 4, minSize=minFaceSize,
                                                                    maxSize=maxFaceSize)
...
def process_frame_with_faces():
    global faceImage, maxFaceSize, color, grey, minFaceSize, faceOutputPath, faces, bestFaceOutputPath, previousBestImage, previousBestImageCount
